            ...
            {orders.map((order) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`px-2 py-1 text-xs rounded font-semibold ${getStatusColor(order.status)}`}>
                    {getStatusText(order.status)}
                  </span>
                </div>

                <div className="mb-2">
                  <p className="text-sm text-gray-600">Capacity: {order.capacity}</p>
                  {order.totalBill ? (
                    <p className="text-sm font-semibold text-gray-800">Total: ₹{order.totalBill.toFixed(2)}</p>
                  ) : null}
                </div>
                ...
                {order.status === 'served' && (
                  <div className="mt-3 p-2 bg-orange-50 border border-orange-200 rounded text-center">
                    ...
                    <p className="text-xs text-gray-600 mt-1">Complete payment in Print Bill section</p>
                  </div>
                )}
              </div>
            ))}
